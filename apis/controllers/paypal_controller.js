    ...
    paypal.payment.execute(
        ...
        function (error, payment) {
            if (error) {
                ...
                console.log(payment);
                var rid =
                    randomUUID() + new Date().toISOString().replace(/:/g, '-');
...
const createWithdraw = catchAsync(async (req, res, next) => {
    try {
        const newWithdraw = await withdrawService.createWithDrawRequest(
            { ...req.body },
            req.user.id
        );
...
const withdrawMoney = catchAsync(async (req, res, next) => {
    ...
    const withdrawRequest = await withdrawService.getWithDrawRequestInfo(
        withdrawId
    );
    if (withdrawRequest == null) {
        return res
            .status(400)
            .json({ success: false, message: 'Request withdraw not found' });
    }
        ...
        body: JSON.stringify({
            ...
            application_context: {
                ...
                return_url:
                    ...
                    '/dashboard/reconcile/success?withdrawId=' +
                    withdrawId,
                cancel_url:
                    process.env.FE_DOMAIN + '/dashboard/reconcile/cancel',
            },
            purchase_units: [
                {
                    amount: {
    ...
    if (data != null) {
        ...
        for (let i = 0; i < data.links.length; i++) {
            if (data.links[i].rel === 'payer-action') {
                ...
                res.status(httpStatus.OK).send({ link: data.links[i].href });
            }
        }
    } else {
        return next(
            new CustomError(httpStatus.INTERNAL_SERVER_ERROR, 'Payment error')
        );
    }
});
...
const capturePaymentOrder = catchAsync(async (req, res, next) => {
    ...
    if (data == null) {
        return res
            .status(500)
            .json({ success: false, message: 'Error when deposit money' });
    } else {
        if (data.status == 'COMPLETED') {
            await withdrawService.realDebitAccount(
                withdrawRequest.user,
                withdrawRequest.transactionalMoney
            );
            await withdrawService.deleteWithDrawRequest(withdrawId);
            var rid =
                randomUUID() + new Date().toISOString().replace(/:/g, '-');
            await reconcileService.createReconcile(
                rid,
                withDrawTitle,
                withdrawRequest.transactionalMoney,
                'USD',
